...
function parseDaemonResponse (res) {
  // check if url was send_raw_transaction
  // if (res.config.url.includes('send_raw_transaction')) {
  //   return res.data
  // }
  ...
  if (res.status === 200) {
    ...
    if (json.status === 'OK' || json.status === 'BUSY' || (json.status === 'Failed' && res.config.url.includes('send_raw_transaction'))) {
      return json
    } else {
      const error = new Error('RPC Error!')
      error.code = json.error?.code || 0
      error.message = json.error?.message || json.status
      throw error
    }
  } else {
 ...
 */
   ...
   */
    ...
    * Error: NOT MINING
    * at _request.then (C:\myDev\scala-rpc-js\src\rpcDaemon.js:1418:27)
    ...
    *   at _request.then (C:\myDev\scala-rpc-js\src\rpcDaemon.js:761:25)
